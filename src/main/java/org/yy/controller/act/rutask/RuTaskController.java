package org.yy.controller.act.rutask;
...
public class RuTaskController extends AcBusinessController {
	...
	public Object handle() throws Exception{
		...
		List<PageData>	paramslist = ruprocdefService.getProParams(pd);
		for (int i = 0; i < paramslist.size(); i++) {
			PageData pdA =paramslist.get(i);
			if(pdA.getString("CITE_DEL").equals("Y")) {
				dellist.add(pdA);
			}
			setVariablesByTaskId(taskId,pdA.getString("FIELD_NAME"),pdA.getString("FIELD_VALUE"));
		}
		if("yes".equals(msg)){								//批准
			...
			List<Task> tasklist2 = findMyTaskList(pd.getString("PROC_INST_ID_"));
			for (int i = 0; i < tasklist2.size(); i++) {
				...
				if(taskId.equals(task.getId())){
					...
					map1.put("审批结果", "【并行自动驳回】,"+Jurisdiction.getName()+"fh,驳回");		//审批结果
					setVariablesByTaskIdAsMap(task.getId(),map1);			//设置流程变量
					setVariablesByTaskId(task.getId(),"RESULT","驳回");
		...
		try{
			PageData pdNum =ruprocdefService.getVarnum(pd);
			int countN=(Integer)pdNum.get("NUM");
			if(countN>0) {
				for (int i = 0; i < dellist.size(); i++) {
					PageData pdA =dellist.get(i);
					removeVariablesByPROC_INST_ID_(pd.getString("PROC_INST_ID_"),pdA.getString("FIELD_NAME"));
				}
				 ruprocdefService.updProParams(pd);
