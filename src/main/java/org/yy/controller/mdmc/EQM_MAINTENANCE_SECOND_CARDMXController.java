 ...
 */
@Controller
@RequestMapping("/eqm_maintenance_second_cardmx")
public class EQM_MAINTENANCE_SECOND_CARDMXController extends BaseController {
	...
	public Object add() throws Exception{
		...
		pd = this.getPageData();
		pd.put("EQM_MAINTENANCE_SECOND_CARDMX_ID", this.get32UUID());	//主键
		eqm_maintenance_second_cardmxService.save(pd);
	...
	public Object delete() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		eqm_maintenance_second_cardmxService.delete(pd);
		map.put("result", errInfo);				//返回结果
		return map;
	}
	...
	public Object edit() throws Exception{
		...
		eqm_maintenance_second_cardmxService.edit(pd);
		map.put("result", errInfo);				//返回结果
		return map;
	}
	...
	public Object list(Page page) throws Exception{
		...
		page.setPd(pd);
		List<PageData>	varList = eqm_maintenance_second_cardmxService.list(page);	//列出EQM_MAINTENANCE_SECOND_CARDMX列表
	...
	public Object goEdit() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		pd = eqm_maintenance_second_cardmxService.findById(pd);	//根据ID读取
		map.put("pd", pd);
		map.put("result", errInfo);						//返回结果
		return map;
	}	
	...
	public Object deleteAll() throws Exception{
		...
		if(Tools.notEmpty(DATA_IDS)){
			...
			eqm_maintenance_second_cardmxService.deleteAll(ArrayDATA_IDS);
			errInfo = "success";
		}else{
			errInfo = "error";
		}
	...
	public ModelAndView exportExcel() throws Exception{
		...
		titles.add("日期");	//5
		titles.add("保养内容反馈");	//6
		titles.add("保养项月模板主键");	//7
		titles.add("保养项月模板保养项目");	//8
		titles.add("保养结果记录");	//9
		titles.add("创建人");	//10
		titles.add("创建时间");	//11
		titles.add("最后修改人");	//12
		...
		List<PageData> varOList = eqm_maintenance_second_cardmxService.listAll(pd);
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var7", varOList.get(i).getString("EQM_MAINTENANCE_ITEM_MONTH_ID"));	    //7
			vpd.put("var8", varOList.get(i).getString("FMAINTENANCE_ITEM"));	    //8
			vpd.put("var9", varOList.get(i).getString("MAINTAIN_RESULT_RECORD"));	    //9
