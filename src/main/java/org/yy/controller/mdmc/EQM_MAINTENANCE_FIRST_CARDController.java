...
import org.yy.service.mdmc.EQM_MAINTENANCE_FIRST_CARDService;
 ...
 */
@Controller
@RequestMapping("/eqm_maintenance_first_card")
public class EQM_MAINTENANCE_FIRST_CARDController extends BaseController {
	
	@Autowired
	private EQM_MAINTENANCE_FIRST_CARDService eqm_maintenance_first_cardService;
	...
	public Object add() throws Exception{
		...
		pd = this.getPageData();
		try {
			String EQM_MAINTENANCE_PLAN_WEEK_ID=pd.getString("EQM_MAINTENANCE_PLAN_WEEK_ID");//保养周计划主键
			String PRINCIPAL=pd.getString("PRINCIPAL");
			String IMMEDIATE_SUPERIOR=pd.getString("IMMEDIATE_SUPERIOR");
			String REMARK=pd.getString("REMARK");
			Integer TIME=Integer.valueOf(pd.getString("TIME"));//次数
			
			PageData pdweek=eqm_maintenance_plan_weekService.findById(pd);
			
			String FEQM_ID=pdweek.getString("FEQM_ID");//设备主键
			String FCLASS_ID=pdweek.getString("FEQM_CLASS_ID");//设备类主键
			String SINGLE_NUMBER=pdweek.getString("FRECEIPT_NUMBER");//本单号
			String MACHINE_NAME=pdweek.getString("FEQM_NAME");//设备名称
			String MACHINE_NUMBER=pdweek.getString("FEQM_CODE");//设备编号
			pd.put("FCLASS_ID", FCLASS_ID);
			
			PageData pdc=eqm_maintenance_item_mainService.findByFclassId(pd);
			List<PageData> pdweekmx=eqm_maintenance_item_weekService.findByFatherId(pdc);
			savepd.put("EQM_MAINTENANCE_FIRST_CARD_ID", this.get32UUID());	//主键
			savepd.put("EQM_MAINTENANCE_PLAN_WEEK_ID", EQM_MAINTENANCE_PLAN_WEEK_ID);
			...
			savepd.put("PRINCIPAL", PRINCIPAL);
			savepd.put("IMMEDIATE_SUPERIOR", IMMEDIATE_SUPERIOR);
			...
			eqm_maintenance_first_cardService.save(savepd);
			if(pdweekmx.size()>0){
				for (int i = 0; i < pdweekmx.size(); i++) {
					PageData pddate=pdweekmx.get(i);
					String FMAINTENANCE_CONTENT=pddate.getString("FMAINTENANCE_CONTENT");
					String EQM_MAINTENANCE_ITEM_WEEK_ID=pddate.getString("EQM_MAINTENANCE_ITEM_WEEK_ID");
					PageData pdsavemx=new PageData();
					pdsavemx.put("EQM_MAINTENANCE_FIRST_CARDMX_ID", this.get32UUID());
					pdsavemx.put("EQM_MAINTENANCE_FIRST_CARD_ID", savepd.getString("EQM_MAINTENANCE_FIRST_CARD_ID"));
					pdsavemx.put("EQM_MAINTENANCE_PLAN_WEEK_ID", EQM_MAINTENANCE_PLAN_WEEK_ID);
					...
					pdsavemx.put("FMAINTENANCE_CONTENT", FMAINTENANCE_CONTENT);
					pdsavemx.put("EQM_MAINTENANCE_ITEM_WEEK_ID", EQM_MAINTENANCE_ITEM_WEEK_ID);
					...
					eqm_maintenance_first_cardmxService.save(pdsavemx);
				}
			}
			List<PageData> list=eqm_maintenance_first_cardService.listAll(pd);
			if(list.size()>0){
				PageData pdedit=new PageData();
					Integer FMAINTENANCE_COUNT = list.size();
					String strSql="UPDATE MDMC_EQM_MAINTENANCE_PLAN_WEEK SET FMAINTENANCE_COUNT"+TIME+" = "+FMAINTENANCE_COUNT+",FSTATE"+TIME+" = '已保养' where EQM_MAINTENANCE_PLAN_WEEK_ID = '"+EQM_MAINTENANCE_PLAN_WEEK_ID+"'";
					pdedit.put("strSql", strSql);
					eqm_maintenance_plan_weekService.editState(pdedit);
			}
		} catch (Exception e) {
			errInfo="error";
			e.printStackTrace();
	...
	public Object delete() throws Exception{
		...
		pd = this.getPageData();
		try{
			String EQM_MAINTENANCE_PLAN_WEEK_ID=pd.getString("EQM_MAINTENANCE_PLAN_WEEK_ID");
			Integer TIME=Integer.valueOf(pd.getString("TIME"));//次数
			eqm_maintenance_first_cardmxService.deleteFather(pd);
			eqm_maintenance_first_cardService.delete(pd);
			List<PageData> list=eqm_maintenance_first_cardService.listAll(pd);
			if(list.size()>0){
				PageData pdedit=new PageData();
				Integer FMAINTENANCE_COUNT = list.size();
				...
				PageData pdedit=new PageData();
				Integer FMAINTENANCE_COUNT = list.size();
				String strSql="UPDATE MDMC_EQM_MAINTENANCE_PLAN_WEEK SET FMAINTENANCE_COUNT"+TIME+" = "+FMAINTENANCE_COUNT+",FSTATE"+TIME+" = '未保养' where EQM_MAINTENANCE_PLAN_WEEK_ID = '"+EQM_MAINTENANCE_PLAN_WEEK_ID+"'";
				pdedit.put("strSql", strSql);
				eqm_maintenance_plan_weekService.editState(pdedit);
			}
		} catch(Exception e){
	...
	public Object edit() throws Exception{
		...
		eqm_maintenance_first_cardService.edit(pd);
		map.put("result", errInfo);				//返回结果
		return map;
	}
	...
	public Object editState() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		eqm_maintenance_first_cardService.editState(pd);
		map.put("result", errInfo);				//返回结果
		return map;
	}
	/**
	...
	public Object editState1() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		eqm_maintenance_first_cardService.editState1(pd);
		map.put("result", errInfo);				//返回结果
		return map;
	}
	...
	public Object list(Page page) throws Exception{
		...
		page.setPd(pd);
		List<PageData> varList = eqm_maintenance_first_cardService.list(page);	//列出EQM_MAINTENANCE_FIRST_CARD列表
		if(varList.size()>0){
			for (int i=0;i<varList.size();i++) {
				PageData pd1=varList.get(i);
				List<PageData>	varList1=eqm_maintenance_first_cardmxService.listAll(pd1);
	...
	public Object goEdit()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		pd = eqm_maintenance_first_cardService.findById(pd);	//根据ID读取
		map.put("pd", pd);
		map.put("result", errInfo);						//返回结果
		return map;
	}	
	...
	public ModelAndView exportExcel() throws Exception{
		...
		List<PageData> varOList = eqm_maintenance_first_cardService.listAll(pd);
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var10", varOList.get(i).getString("IMMEDIATE_SUPERIOR"));	    //10
