 ...
 */
@Controller
@RequestMapping("/eqm_maintenance_item_quarter")
public class EQM_MAINTENANCE_ITEM_QUARTERController extends BaseController {
	...
	public Object add() throws Exception{
		...
		pd = this.getPageData();
		pd.put("EQM_MAINTENANCE_ITEM_QUARTER_ID", this.get32UUID());	//主键
		...
		eqm_maintenance_item_quarterService.save(pd);
	...
	public Object delete() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		eqm_maintenance_item_quarterService.delete(pd);
		map.put("result", errInfo);				//返回结果
		return map;
	}
	...
	public Object edit() throws Exception{
		...
		pd.put("LAST_MODIFIER", Jurisdiction.getUsername());
		pd.put("LAST_MODIFIED_TIME", Tools.date2Str(new Date()));
		eqm_maintenance_item_quarterService.edit(pd);
		map.put("result", errInfo);				//返回结果
		return map;
	}
	...
	public Object list(Page page) throws Exception{
		...
		page.setPd(pd);
		List<PageData>	varList = eqm_maintenance_item_quarterService.list(page);	//列出EQM_MAINTENANCE_ITEM_QUARTER列表
	...
	public Object goEdit() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		pd = eqm_maintenance_item_quarterService.findById(pd);	//根据ID读取
		map.put("pd", pd);
		map.put("result", errInfo);						//返回结果
		return map;
	}	
	...
	public Object deleteAll() throws Exception{
		...
		if(Tools.notEmpty(DATA_IDS)){
			...
			eqm_maintenance_item_quarterService.deleteAll(ArrayDATA_IDS);
			errInfo = "success";
		}else{
			errInfo = "error";
		}
	...
	public ModelAndView exportExcel() throws Exception{
		...
		List<PageData> varOList = eqm_maintenance_item_quarterService.listAll(pd);
		List<PageData> varList = new ArrayList<PageData>();
		for(int i=0;i<varOList.size();i++){
			PageData vpd = new PageData();
			vpd.put("var1", varOList.get(i).getString("FMAINTENANCE_ITEM"));	    //1
