 ...
 */
@Controller
@RequestMapping("/eqm_maintenance_plan_week")
public class EQM_MAINTENANCE_PLAN_WEEKController extends BaseController {
	...
	//@RequiresPermissions("eqm_maintenance_plan_week:add")
	@ResponseBody
	public Object add() throws Exception{
		...
		pd = this.getPageData();
		PageData pdNum=eqm_maintenance_plan_weekService.getNUM(pd);	//根据选中月份（2020-09）查找本月创建过周计划数量
		if(null != pdNum && Integer.parseInt(pdNum.get("NUM").toString())!=0) {
			errInfo = "fail1";//选中月份已创建过计划
		}else {
			
		List<PageData>	eqmList =new ArrayList<PageData>();
		String deptC=Jurisdiction.getDEPARTMENT_ID();
		//String deptC="8a123ea49e1d4b57a08db9861485529d";
		String workshop=getDept(deptC);
		if(workshop !="") {
			PageData pdEqm=new PageData();
			pdEqm.put("workshop", workshop);
			eqmList = eqm_maintenance_plan_weekService.getEqmList(pdEqm);//根据车间部门id查设备列表	
		}
		for(int j=0;j<eqmList.size();j++) {
			PageData empPd=eqmList.get(j);
			pdSave.put("EQM_MAINTENANCE_PLAN_WEEK_ID", this.get32UUID());	//主键
			pdSave.put("FEQM_ID", empPd.getString("EQM_BASE_ID"));	
			pdSave.put("FEQM_CLASS_ID", empPd.getString("EQM_CLASS_ID"));
			pdSave.put("FRECEIPT_NUMBER", pd.getString("month"));//选中月份，2020-09
			...
			pdSave.put("FMAINTENANCE_CLASS", "一级保养");
			...
			pdSave.put("FBILLNO", "MAW"+Tools.date2Str(new Date(),"yyyyMMddHHmmss"));	//编号
			for(int m=1;m<=5;m++) {
				pdSave.put("FMAINTENANCE_COUNT"+m, 0);//保养次数
				pdSave.put("FSTATE"+m, "未保养");//保养状态
			}
			pdSave.put("FEXTEND1", "新增");//创建方式
			eqm_maintenance_plan_weekService.save(pdSave);
		}
	...
	@RequiresPermissions("eqm_maintenance_plan_week:del")
	@ResponseBody
	public Object delete() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		eqm_maintenance_plan_weekService.delete(pd);
	...
	@RequiresPermissions("eqm_maintenance_plan_week:edit")
	@ResponseBody
	public Object edit() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		eqm_maintenance_plan_weekService.edit(pd);
		map.put("result", errInfo);
		return map;
	}
	...
	@RequiresPermissions("eqm_maintenance_plan_week:list")
	@ResponseBody
	public Object list(Page page) throws Exception{
		...
		if(Tools.notEmpty(KEYWORDS1)) {
			...
			pd.put("KEYWORDS1", Tools.date2Str(new Date(),"yyyy-MM"));	//编号
		}
		...
		List<PageData>	varList = eqm_maintenance_plan_weekService.list(page);	//列出EQM_MAINTENANCE_PLAN_WEEK列表
	 ...
	 /**周计划待选择月份列表
		...
		@ResponseBody
		public Object getMonthList() throws Exception{
			Map<String,Object> map = new HashMap<String,Object>();
			String errInfo = "success";
			PageData pd = new PageData();
			pd = this.getPageData();
			//pd.put("FYEAR", Tools.date2Str(new Date(),"yyyy"));	
			List<PageData>	varList = eqm_maintenance_plan_weekService.getMonthList(pd);	
			map.put("varList", varList);
			map.put("result", errInfo);
			return map;
		}	
		...
		public Object getNoList() throws Exception{
			...
			PageData pd = new PageData();
			pd = this.getPageData();
			List<PageData>	varList = eqm_maintenance_plan_weekService.getNoList(pd);	
			map.put("varList", varList);
			map.put("result", errInfo);
			return map;
		}	
		...
		public Object getClassList() throws Exception{
			...
			List<PageData>	varList = eqm_maintenance_plan_weekService.getClassList(pd);	
			map.put("varList", varList);
			map.put("result", errInfo);
			return map;
		}	
		...
		public Object getWsList() throws Exception{
			...
			PageData pd = new PageData();
			pd = this.getPageData();
			List<PageData>	varList = eqm_maintenance_plan_weekService.getWsList(pd);	
			map.put("varList", varList);
			map.put("result", errInfo);
			return map;
		}	
	...
	public Object goEdit() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		pd = eqm_maintenance_plan_weekService.findById(pd);	//根据ID读取
		map.put("pd", pd);
		map.put("result", errInfo);
		return map;
	}	
	...
	public Object deleteAll() throws Exception{
		...
		if(Tools.notEmpty(DATA_IDS)){
			...
			eqm_maintenance_plan_weekService.deleteAll(ArrayDATA_IDS);
			errInfo = "success";
		}else{
			errInfo = "error";
		}
	...
	public ModelAndView exportExcel() throws Exception{
		...
		titles.add("第四周");	//9
		titles.add("第五周");	//10
		titles.add("第一周保养次数");	//11
		titles.add("第二周保养次数");	//12
		titles.add("第三周保养次数");	//13
		titles.add("第四周保养次数");	//14
		titles.add("第五周保养次数");	//15
		titles.add("第一周保养状态");	//16
		titles.add("第二周保养状态");	//17
		titles.add("第三周保养状态");	//18
		titles.add("第四周保养状态");	//19
		titles.add("第五周保养状态");	//20
		titles.add("编号");	//21
		titles.add("备注");	//22
		titles.add("创建人");	//23
		titles.add("创建时间");	//24
		...
		List<PageData> varOList = eqm_maintenance_plan_weekService.listAll(pd);
		List<PageData> varList = new ArrayList<PageData>();
		for(int i=0;i<varOList.size();i++){
			PageData vpd = new PageData();
			vpd.put("var1", varOList.get(i).getString("FRECEIPT_NUMBER"));	    //1
			vpd.put("var2", varOList.get(i).getString("FWORKSHOP_ID"));	    //2
			vpd.put("var3", varOList.get(i).getString("FEQM_NAME"));	    //3
			vpd.put("var4", varOList.get(i).getString("FEQM_CODE"));	    //4
			vpd.put("var5", varOList.get(i).getString("FMAINTENANCE_CLASS"));	    //5
			vpd.put("var6", varOList.get(i).getString("FDATE1"));	    //6
			vpd.put("var7", varOList.get(i).getString("FDATE2"));	    //7
			vpd.put("var8", varOList.get(i).getString("FDATE3"));	    //8
			vpd.put("var9", varOList.get(i).getString("FDATE4"));	    //9
			vpd.put("var10", varOList.get(i).getString("FDATE5"));	    //10
			vpd.put("var11", varOList.get(i).get("FMAINTENANCE_COUNT1").toString());	//11
			vpd.put("var12", varOList.get(i).get("FMAINTENANCE_COUNT2").toString());	//12
			vpd.put("var13", varOList.get(i).get("FMAINTENANCE_COUNT3").toString());	//13
			vpd.put("var14", varOList.get(i).get("FMAINTENANCE_COUNT4").toString());	//14
			vpd.put("var15", varOList.get(i).get("FMAINTENANCE_COUNT5").toString());	//15
			...
			vpd.put("var20", varOList.get(i).getString("FSTATE5"));	    //20
			vpd.put("var21", varOList.get(i).getString("FBILLNO"));	    //21
			vpd.put("var22", varOList.get(i).getString("FNOTE"));	    //22
			vpd.put("var23", varOList.get(i).getString("FCREATOR"));	    //23
			vpd.put("var24", varOList.get(i).getString("CREATE_TIME"));	    //24
			vpd.put("var25", varOList.get(i).getString("LAST_MODIFIER"));	    //25
			vpd.put("var26", varOList.get(i).getString("LAST_MODIFIED_TIME"));	    //26
	...
	@RequiresPermissions("eqm_maintenance_plan_week_run:list")
	@ResponseBody
	...
	@RequiresPermissions("eqm_maintenance_plan_week_history:list")
	@ResponseBody
	...
	public Object updateEqm() throws Exception{
		...
		if(workshop !="") {
			PageData pdEqm=new PageData();
			pdEqm.put("workshop", workshop);
			pdEqm.put("FTYPE", "WEEK_UP");//区分计划类型
