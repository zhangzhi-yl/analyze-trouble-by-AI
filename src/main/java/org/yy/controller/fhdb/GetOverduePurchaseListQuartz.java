...
@Controller
public class GetOverduePurchaseListQuartz extends BaseController implements Job{
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		List<PageData> overduePurchaseList = earlyWarningService.getOverduePurchaseList(pd);
		for (PageData pageData : overduePurchaseList) {
			...
			pdNotice.put("FContent","物料"+pageData.getString("MAT_NAME")+"距离最后购买期限已经超期"+Integer.parseInt(pageData.get("diff").toString())*(-1)+"天了，请注意");
