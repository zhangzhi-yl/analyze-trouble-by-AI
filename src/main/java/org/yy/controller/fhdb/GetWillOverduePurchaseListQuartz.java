...
@Controller
public class GetWillOverduePurchaseListQuartz extends BaseController implements Job{
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		List<PageData> willOverduePurchaseList = earlyWarningService.getWillOverduePurchaseList(pd);
		for (PageData pageData : willOverduePurchaseList) {
			...
			pdNotice.put("FContent","物料"+pageData.getString("MAT_NAME")+"距离最后购买期限还剩"+Integer.parseInt(pageData.get("diff").toString())+"天，请注意");
			pdNotice.put("FTitle", "物料采购预警");// 消息标题
			pdNotice.put("LinkIf", "no");// 是否跳转页面
			pdNotice.put("DataSources", "物料采购预警");// 数据来源
