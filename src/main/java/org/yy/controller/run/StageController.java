...
public class StageController extends BaseController {
	...
	/**
	 * 参数TYPE:P1上级提请 需要RUN_O_ID P2本级提请 不同TYPE要通过不同方式获取的BEGIN_NODE值 P1
	 * 
	 * @return pd
	 * @throws Exception
	 */

	@RequestMapping(value = "/run/nodes")
	@ResponseBody
	public Object nodes() throws Exception {
		...
		String RUN_O_ID = pd.getString("RUN_O_ID");
		String NODE_ID = pd.getString("NODE_ID");
		String MASTERPLAN_ID = pd.getString("MASTERPLAN_ID");
		String SUBPLAN_ID = pd.getString("SUBPLAN_ID");
		try {
			getNode2(TYPE, RUN_O_ID, NODE_ID, MASTERPLAN_ID, SUBPLAN_ID);
		} catch (Exception e) {
			result = "500";
		} finally {
				...
				if (num > 1) {// 说明有并行节点未执行完的节点
					...
					for (PageData pageData : list) {
						if (pageData.getString("NODE_TYPE").equals("end round")) {
							...
							// getNode1("P2", firstNodePd.getString("NODE_ID"),
							// MASTERPLAN_ID);
							// System.out.println("已经运转到第2级最后一级节点-下一步应该寻找上一层级节点了");
							
							// P1 工单结束掉，更新结束时间
							String planID = pd.getString("SUBPLAN_ID");
							PageData  pData = new PageData();
							pData.put("PlanningWorkOrder_ID", planID);
							PageData plan1 = pwoService.findById(pData);
							plan1.put("FState", "结束");
							pwoService.edit(plan1);
							...
							if (Tools.notEmpty(NEXT_SUBPLAN_ID)) {
								...
								for (PageData pageData2 : listAll) {
									...
									pageData2.put("ActualBeginTime", Tools.date2Str(new Date()));
