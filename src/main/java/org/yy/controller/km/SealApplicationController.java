...
public class SealApplicationController extends BaseController {
	...
	@RequiresPermissions("SealApplication:add")
	@ResponseBody
	public Object add() throws Exception{
		...
		pd = this.getPageData();
		pd.put("SealApplication_ID", this.get32UUID());	//主键
		...
		SealApplicationService.save(pd);
	...
	@RequiresPermissions("SealApplication:del")
	@ResponseBody
	public Object delete() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		SealApplicationService.delete(pd);
	...
	@RequiresPermissions("SealApplication:edit")
	@ResponseBody
	public Object edit() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		SealApplicationService.edit(pd);
		map.put("result", errInfo);
		return map;
	}
	...
	@RequiresPermissions("SealApplication:list")
	@ResponseBody
	public Object list(Page page) throws Exception{
		...
		page.setPd(pd);
		List<PageData>	varList = SealApplicationService.list(page);	//列出SealApplication列表
		for(int i=0;i<varList.size();i++) {
			String FCopyMan=varList.get(i).getString("FCopyMan");
			if(FCopyMan != null && !"".equals(FCopyMan)) {
				PageData pdCopy=SealApplicationService.getCopyName(varList.get(i));
					if(pdCopy != null) {
	...
	public Object goEdit() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		pd = SealApplicationService.findById(pd);	//根据ID读取
		String FCopyMan=pd.getString("FCopyMan");
		if(FCopyMan != null && !"".equals(FCopyMan)) {
			PageData pdCopy=SealApplicationService.getCopyName(pd);
				if(pdCopy != null) {
	...
	public Object deleteAll() throws Exception{
		...
		if(Tools.notEmpty(DATA_IDS)){
			...
			SealApplicationService.deleteAll(ArrayDATA_IDS);
			errInfo = "success";
		}else{
			errInfo = "error";
		}
	...
	public ModelAndView exportExcel() throws Exception{
		...
		titles.add("状态");	//9
		titles.add("审核进度");	//10
		titles.add("提交审批时间");	//11
		titles.add("提交审批人");	//12
		titles.add("流程ID");	//13
		titles.add("抄送人");	//14
		dataMap.put("titles", titles);
		List<PageData> varOList = SealApplicationService.listAll(pd);
		List<PageData> varList = new ArrayList<PageData>();
		for(int i=0;i<varOList.size();i++){
			PageData vpd = new PageData();
			vpd.put("var1", varOList.get(i).getString("FType"));	    //1
			vpd.put("var2", varOList.get(i).getString("Fbillno"));	    //2
			vpd.put("var3", varOList.get(i).getString("FBorrowTime"));	    //3
			vpd.put("var4", varOList.get(i).getString("FReturnTime"));	    //4
			vpd.put("var5", varOList.get(i).getString("FReason"));	    //5
			vpd.put("var6", varOList.get(i).getString("FNOTE"));	    //6
			vpd.put("var7", varOList.get(i).getString("FCreatePerson"));	    //7
			vpd.put("var8", varOList.get(i).getString("FCreateTime"));	    //8
			vpd.put("var9", varOList.get(i).getString("RUN_STATUS"));	    //9
			vpd.put("var10", varOList.get(i).getString("GENERATE_PROGRESS"));	    //10
			vpd.put("var11", varOList.get(i).getString("FPostTime"));	    //11
			vpd.put("var12", varOList.get(i).getString("FPostMan"));	    //12
			vpd.put("var13", varOList.get(i).getString("PROC_INST_ID_"));	    //13
			vpd.put("var14", varOList.get(i).getString("FCopyMan"));	    //14
			varList.add(vpd);
		}
	...
	public Object setStarts() throws Exception{
		...
		pdBack.put("SrartKeyId", "KEY_SealApplication");
		...
		if(strs.length >0) {
			...
			SealApplicationService.editAudit(pd);
		}else {
			errInfo="fail1";
		}
		map.put("pd", pdBack);
