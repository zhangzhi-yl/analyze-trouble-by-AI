 ...
 */
@Controller
@RequestMapping("/kmcustomer")
public class KMCustomerController extends BaseController {
	...
	public Object add() throws Exception{
		...
		num = customerService.findCountByCode(pd);
		...
		operationrecordService.add("","客户","添加",pd.getString("Customer_ID"),staffid,"添加客户");
	...
	public Object delete() throws Exception{
		...
		operationrecordService.add("","客户","删除",pd.getString("Customer_ID"),staffid,"删除客户");
	...
	public Object edit() throws Exception{
		...
		operationrecordService.add("","客户","修改",pd.getString("Customer_ID"),staffid,"修改客户");
		map.put("result", errInfo);
		return map;
	}
	...
	public ModelAndView exportExcel() throws Exception{
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var3", varOList.get(i).get("CustomerRating").toString());	//3
			vpd.put("var4", varOList.get(i).getString("TType"));	    		//4
			...
			vpd.put("var8", varOList.get(i).getString("Industry"));	    		//8
			vpd.put("var9", varOList.get(i).getString("TaxRegistrationNum"));	//9
			vpd.put("var10", varOList.get(i).getString("BankAccount"));	    	//10
			vpd.put("var11", varOList.get(i).getString("FCity"));	    		//11
			vpd.put("var12", varOList.get(i).getString("LegalRepresentative"));	//12
			vpd.put("var13", varOList.get(i).getString("FStatus"));	    		//13
			vpd.put("var14", varOList.get(i).get("CustomerCreditRating").toString());	//14
			vpd.put("var15", varOList.get(i).getString("Describe"));	    	//15
			varList.add(vpd);
		}
	...
	public Object getCusTomerList() throws Exception{
		...
		if(Tools.notEmpty(KEYWORDS))pd.put("KEYWORDS", KEYWORDS.trim());
		List<PageData>	varList = customerService.getCusTomerList(pd);	//列出Customer列表
		map.put("varList", varList);
		map.put("pd", pd);
		map.put("result", errInfo);
		return map;
