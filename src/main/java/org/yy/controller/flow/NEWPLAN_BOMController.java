 ...
 */
@Controller
@RequestMapping("/NEWPLAN_BOM")
public class NEWPLAN_BOMController extends BaseController {
	...
	@RequiresPermissions("NEWPLAN_BOM:add")
	@ResponseBody
	public Object add() throws Exception{
		...
		pd = this.getPageData();
		pd.put("NEWPLAN_BOM_ID", this.get32UUID());	//主键
		NEWPLAN_BOMService.save(pd);
	...
	@RequiresPermissions("NEWPLAN_BOM:del")
	@ResponseBody
	...
	@RequiresPermissions("NEWPLAN_BOM:edit")
	@ResponseBody
	...
	@RequiresPermissions("NEWPLAN_BOM:list")
	@ResponseBody
	public Object list(Page page) throws Exception{
		...
		page.setPd(pd);
		List<PageData>	varList = NEWPLAN_BOMService.list(page);	//列出NEWPLAN_BOM列表
	...
	public Object goEdit() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		pd = NEWPLAN_BOMService.findById(pd);	//根据ID读取
		map.put("pd", pd);
		map.put("result", errInfo);
		return map;
	}	
	...
	public Object deleteAll() throws Exception{
		...
		if(Tools.notEmpty(DATA_IDS)){
			...
			NEWPLAN_BOMService.deleteAll(ArrayDATA_IDS);
			errInfo = "success";
		}else{
			errInfo = "error";
		}
	...
	public ModelAndView exportExcel() throws Exception{
		...
		titles.add("创建时间");	//10
		titles.add("最后变更人");	//11
		titles.add("最后变更时间");	//12
		titles.add("PH_描述");	//13
		titles.add("备注15");	//14
		dataMap.put("titles", titles);
		List<PageData> varOList = NEWPLAN_BOMService.listAll(pd);
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var14", varOList.get(i).getString("MASTER_PLAN_ID"));	    //14
			varList.add(vpd);
		}
