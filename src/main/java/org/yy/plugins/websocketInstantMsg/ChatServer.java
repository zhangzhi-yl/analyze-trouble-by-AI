package org.yy.plugins.websocketInstantMsg;
...
import org.yy.service.fhim.ImstateService;
...
public class ChatServer extends WebSocketServer{
	...
	public void onMessage(WebSocket conn, String message){
		...
		if(null != message && message.startsWith(",yysystem-msg,")){			 		//用户发送的数据消息
			this.usersendmsg(message.replaceFirst(",yysystem-msg,", ""), conn);
		}else if(null != message && message.startsWith(",yysystem-join,")){			//用户上线消息
			this.userjoin(message.replaceFirst(",yysystem-join,", ""),conn); 
		}else if(null != message && message.startsWith(",yysystem-leave,")){			//用户下线消息
			this.userLeave(conn);
		}else if(null != message && message.startsWith(",yysystem-online,")){		//用户设置在线消息
			this.useronline(message.replaceFirst(",yysystem-online,", ""),conn); 
		}else if(null != message && message.startsWith(",yysystem-hide,")){			//用户设置隐身消息
			this.userhide(message.replaceFirst(",yysystem-hide,", ""),conn); 
		}else if(null != message && message.startsWith(",yysystem-applyfriend,")){	//申请好友消息
			this.applyfriend(message.replaceFirst(",yysystem-applyfriend,", ""),conn); 
		}else if(null != message && message.startsWith(",yysystem-remove,")){		//用户删除好友消息
			this.userremove(message.replaceFirst(",yysystem-remove,", ""),conn); 
		}else if(null != message && message.startsWith(",yysystem-agreefriend,")){	//同意添加好友
			this.agreefriend(message.replaceFirst(",yysystem-agreefriend,", ""),conn); 
		}else if(null != message && message.startsWith(",yysystem-applyqgroup,")){	//申请加群
			this.applyqgroup(message.replaceFirst(",yysystem-applyqgroup,", ""),conn); 
		}else if(null != message && message.startsWith(",yysystem-agreeqgroup,")){	//同意加群
			this.agreeqgroup(message.replaceFirst(",yysystem-agreeqgroup,", ""),conn); 
		}else if(null != message && message.startsWith(",yysystem-kickoutqgroup,")){	//踢出群
			this.kickoutqgroup(message.replaceFirst(",yysystem-kickoutqgroup,", ""),conn); 
		}else if(null != message && message.startsWith(",yysystem-delqgroup,")){		//解散群
			this.delqgroup(message.replaceFirst(",yysystem-delqgroup,", ""),conn); 
		}
