...
@Slf4j
@Component
@ServerEndpoint(value = "/exceNewlSocket/{name}/{gridKey}", configurator = GetHttpSessionConfigurator.class)
public class OnlineExcelNewWebSocketServer {

    private static OnlineExcelNewWebSocketServer onlineExcelWebSocketServer;
     ...
     */
    private static CopyOnWriteArraySet<OnlineExcelNewWebSocketServer> webSockets = new CopyOnWriteArraySet<>();
    private static ConcurrentHashMap<String, OnlineExcelNewWebSocketServer> tokenMap = new ConcurrentHashMap<>();
    /**
    ...
    @Autowired
    private IMessageNewProcess messageProcess;
...
//        userId = String.valueOf(httpSession.getAttribute("你的token key"));
    	...
    	if(StrUtil.isNotBlank(message)) {
           ...
           for (String key : tokenMap.keySet()) {
               ...
               if (!key.equals(userId)) {
                   OnlineExcelNewWebSocketServer socketServer = (OnlineExcelNewWebSocketServer) tokenMap.get(key);
    ...
    public boolean equals(Object o) {
        ...
        OnlineExcelNewWebSocketServer that = (OnlineExcelNewWebSocketServer) o;
    ...
    public void sendAllMessage(String message) {
        for (OnlineExcelNewWebSocketServer webSocket : webSockets) {
            try {
