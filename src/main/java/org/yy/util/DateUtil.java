...
/**
 * 说明：日期处理 作者：YuanYes Q356703572 官网：356703572@qq.com
 */
public class DateUtil {
	...
	public static String getAfterDayDate(String dateString
			, String dayCount
			, String dateUnit) {
		int daysInt = Integer.parseInt(dayCount);
		Calendar canlendar = Calendar.getInstance(); // java.util包
		Date date = str2Date(dateString);
		canlendar.setTime(date);
		if ("天".equals(dateUnit)) {
			canlendar.add(Calendar.DATE, daysInt); 
		}
		if ("小时".equals(dateUnit)) {
			canlendar.add(Calendar.HOUR, daysInt); 
		}
		if ("分钟".equals(dateUnit)) {
			canlendar.add(Calendar.MINUTE, daysInt); 
		}
		if ("秒".equals(dateUnit)) {
